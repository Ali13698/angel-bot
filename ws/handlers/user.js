...
export async function initUser(ws, msg) {
  ...
  if (msg.initData) {
    tg = verifyInitData(msg.initData);
  }

  // فقط برای تست — باید توی Runflare متغیر ALLOW_DEV_LOGIN=true باشه
  if (!tg && process.env.ALLOW_DEV_LOGIN === 'true' && msg.user && msg.user.id) {
    tg = msg.user;
    console.warn('[ws] DEV LOGIN — بدون امضا');
  }
