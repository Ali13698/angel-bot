    ...
    if (msg.type === "quick_match") {
      ...
      Matchmaker.add(ws, userId, (type, myEntry, oppEntry) => {
        if (type === 'human') {
          ...
          send(a.ws, { type: "match_found", code, playerIndex: 0, opponent: publicUser(DB._cache?.[b.userId] || null) });
          send(b.ws, { type: "match_found", code, playerIndex: 1, opponent: publicUser(DB._cache?.[a.userId] || null) });
        } else if (type === 'bot') {
          const bot = oppEntry;
