...
app.get("/health", (req, res) => res.json({
  ...
  bots: BotPool.count ? BotPool.count() : 0
}));
  ...
  ws.on("message", (raw) => {
    ...
    if (msg.type === "quick_match") {
      ...
      Matchmaker.add(ws, userId, (type, myEntry, oppEntry) => {
        if (type === 'human') {
          // فقط نفر دومی که callback می‌گیره اتاق رو می‌سازه
          ...
          console.log("[match] human:", code);
        } else if (type === 'bot') {
          // myEntry = کاربر، oppEntry = ربات
          const bot = oppEntry;
          ...
          console.log("[match] bot:", code, "vs", bot.name);
        }
      });

      // ذخیره cbId برای پاسخ بعدی
    ...
    if (msg.type === "game_ended") {
      ...
      if (room.botOpponent) {
        ...
        if (winner === humanId) {
          DB.addWin(humanId, 10, 20);
          BotPool.recordLoss(botId);
        } else if (winner === botId) {
          DB.addLoss(humanId, 2, 5);
          BotPool.recordWin(botId);
        }
