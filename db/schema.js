...
async function safe(sql) {
  try { await query(sql); }
  catch (e) { console.log('[schema skip]', e.message.slice(0, 80)); }
}
...
export async function initTables() {
  ...
  for (const a of alters) await safe(a);
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_ugs_user ON user_game_stats(user_id)`);
  ...
  for (const it of seeds) {
    await safe(`INSERT OR IGNORE INTO shop_items (item_id,item_type,name_fa,price_coins,preview,meta,is_premium) VALUES (?,?,?,?,?,?,?)`);
  }
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_inv_user ON inventory(user_id, item_type)`);
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_tx_user ON transactions(user_id, created_at DESC)`);
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_chat_room ON chat_messages(room_code, created_at DESC)`);

  await safe(`CREATE TABLE IF NOT EXISTS matches (
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_match_players ON matches(player1, player2)`);

  await safe(`CREATE TABLE IF NOT EXISTS friends (
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_fr_u ON friends(user_id, status)`);
  await safe(`CREATE INDEX IF NOT EXISTS idx_fr_f ON friends(friend_id, status)`);
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_likes_to ON likes(to_user_id)`);
  ...
  await safe(`CREATE INDEX IF NOT EXISTS idx_rep_target ON reports(target_id, created_at DESC)`);
  await safe(`CREATE INDEX IF NOT EXISTS idx_rep_reporter ON reports(reporter_id, target_id, created_at DESC)`);
